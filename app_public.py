...
parent -- CLI or the `/train` skill, never through this app). What this
...
its own, so hosting it needs only requirements-public.txt. See app.py's
module docstring for the full local tool. Deploy entrypoint for
Render/Fly/etc: gunicorn app_public:app (see render.yaml).
"""
import sys
